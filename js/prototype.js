...
var Class = (function() {
	...
	function create() {
		...
		for (let i = 0, length = properties.length; i < length; i++) {klass.addMethods(properties[i]);}
...
(function() {
	...
	const DONT_ENUMS = ['toString', 'toLocaleString', 'valueOf',
	...
	function keys(object) {
		...
		if (IS_DONTENUM_BUGGY) {
			for (let i = 0; property = DONT_ENUMS[i]; i++) {
				if (_hasOwnProperty.call(object, property)) {results.push(property);}
			}
		}
...
Object.extend(Function.prototype, (function() {
	...
	function bind(context) {
		...
		const nop = function() {};
		const __method = this, args = slice.call(arguments, 1);
	...
	function curry() {
		...
		const __method = this, args = slice.call(arguments, 0);
		return function() {
			const a = merge(args, arguments);
...
Object.extend(String.prototype, (function() {
	...
	function inspect(useDoubleQuotes) {
		const escapedString = this.replace(/[\x00-\x1f\\]/g, function(character) {
...
(function() {
	...
	function indexOf(item, i) {
		...
		const array = Object(this), length = array.length >>> 0;
...
var Hash = Class.create(Enumerable, (function() {
	...
	function index(value) {
		const match = this.detect(function(pair) {
	...
	function toQueryString() {
		return this.inject([], function(results, pair) {
			const key = encodeURIComponent(pair.key), values = pair.value;
...
Ajax.Request = Class.create(Ajax.Base, {
       ...
       */
		...
		if (typeof this.options.requestHeaders == 'object') {
			...
			if (Object.isFunction(extras.push)) {
				for (let i = 0, length = extras.length; i < length; i += 2) {headers[extras[i]] = extras[i + 1];}
			}
	...
	respondToReadyState: function(readyState) {
		const state = Ajax.Request.Events[readyState], response = new Ajax.Response(this);
...
(function(GLOBAL) {
	...
	function inspect(element) {
		...
		let attribute, value;
		for (const property in INSPECT_ATTRIBUTES) {
   ...
   TABLE_ELEMENT_INNERHTML_BUGGY || LINK_ELEMENT_INNERHTML_BUGGY;
	...
	function replace_IE(element, content) {
		...
		content = Object.toHTML(content);
		const parent = element.parentNode, tagName = parent.tagName.toUpperCase();

		if (tagName in INSERTION_TRANSLATIONS.tags) {
			const nextSibling = Element.next(element);
			const fragments = getContentFromAnonymousElement(
	...
	function insert(element, insertions) {
		...
		for (const position in insertions) {insertContentAt(element, insertions[position], position);}
	...
	function getContentFromAnonymousElement(tagName, html, force) {
		...
		if (workaround) {
			...
			for (let i = t[2]; i--;) {div = div.firstChild;}
		}
		else {
			div.innerHTML = html;
		}
	...
	const PROBLEMATIC_ATTRIBUTE_READING = (function() {
	...
	const PROBLEMATIC_HAS_ATTRIBUTE_WITH_CHECKBOXES = (function() {
   ...
   'accessKey tabIndex encType maxLength readOnly longDesc frameBorder');
	...
	const Methods = {}, ByTag = Element.Methods.ByTag,
   ...
   checkElementPrototypeDeficiency('object');
	...
	function addMethods(methods) {
		...
		const ELEMENT_PROTOTYPE = window.HTMLElement ? HTMLElement.prototype :
			Element.prototype;
...
(function() {
	...
	function getPixelValue(value, property, context) {
		...
		const isPercentage = value.include('%'), isViewport = (context === document.viewport);

		if (/\d/.test(value) && element && element.runtimeStyle && !(isPercentage && isViewport)) {
			const style = element.style.left, rStyle = element.runtimeStyle.left;
	...
	Element.Layout = Class.create(Hash, {
		...
		_begin: function() {
			...
			if (width && (positionedWidth === width)) {
				...
				const parent = element.parentNode, pLayout = $(parent).getLayout();
	...
	function makePositioned(element) {
		element = $(element);
		const position = Element.getStyle(element, 'position'), styles = {};
	...
	function makeClipping(element) {
		...
		if (Object.isUndefined(madeClipping)) {
			const overflow = Element.getStyle(element, 'overflow');
	...
	function clonePosition(element, source, options) {
		...
		if (options.setLeft || options.setTop) {
			...
			delta = [0, 0];
			if (Element.getStyle(element, 'position') === 'absolute') {
				const parent = Element.getOffsetParent(element);
...
(function() {
	...
	const IS_OLD_OPERA = Prototype.Browser.Opera &&
...
Form.Methods = {
	...
	findFirstElement: function(form) {
		const elements = $(form).getElements().findAll(function(element) {
			return element.type != 'hidden' && !element.disabled;
		});
		const firstByIndex = elements.findAll(function(element) {
